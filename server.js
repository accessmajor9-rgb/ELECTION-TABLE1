...
button{width:100%;margin-top:12px;padding:18px;background:#0a1931;color:#fff;border:none;border-radius:18px;font-weight:800;cursor:pointer;font-size:15px}
...
// ========== TABLE 2 - ONLY CANDIDATES ==========
app.get('/table2',(req,res)=>{
 let t=(req.query.token||'').trim();
 if(!t ||!issuedTokens.has(t)) return res.send(`${HEAD}<div class="card"><div class="bottom"><h3>Invalid Token!</h3><p>Go back to Table 1 to verify ID</p><a href="/"><button>← Back to Table 1</button></a></div></div></body></html>`);
 ...
 res.send(`${HEAD}<div class="card"><div class="top"><div class="title" style="font-size:26px">Table 2<br>Voting Desk</div><div class="sub">Token Verified ✅ | Choose Candidate</div></div><div class="bottom"><div class="label">SELECT YOUR PREFERRED CANDIDATE</div><button class="cand" onclick="vote('A')">Candidate A - Vote</button><button class="cand" onclick="vote('B')">Candidate B - Vote</button><button class="cand" onclick="vote('C')">Candidate C - Vote</button><div style="text-align:center;margin-top:14px;font-size:10px;color:#94a3b8">1 TOKEN = 1 VOTE | SECURE</div></div></div>
 <script>
 async function vote(c){
  ...
  let d=await r.json();
  if(d.error) return alert(d.error);
  document.body.innerHTML=\`${HEAD}<div class="card"><div class="bottom"><div class="success"><div style="font-size:40px">✅</div><h2 style="color:#065f46">Vote Successful!</h2><p>You voted for <b>\${c}</b></p><p style="font-size:13px;color:#64748b">Please leave the booth.<br>1 vote per person.<br>Token burned.</p><a href="/results"><button style="background:#0a1931;margin-top:16px">View Dashboard (Admin Only)</button></a><a href="/"><button style="background:#f1f5f9;color:#0a1931;margin-top:10px;border:2px solid #e2e8f0">← Back to Table 1</button></a></div></div></div></body></html>\`;
 }
 </script></body></html>`);
});
...
app.get('/admin',(req,res)=>{
 ...
 <a href="/table2?token="><button style="background:#334155;margin-top:8px">→ GO TO TABLE 2</button></a>
 ...
 <div id="voters" style="display:none;margin-top:12px;background:#fff;border:2px solid #e2e8f0;border-radius:12px;padding:12px;max-height:220px;overflow:auto"><div style="font-weight:700;font-size:11px;margin-bottom:8px">SAVED TOKEN LIST</div>${listHtml}<button onclick="let t=\`${voterLog.map(v=>v.id+','+v.token+','+(v.voted?'VOTED-'+v.candidate:'NOT VOTED')).join('\\n')}\`; navigator.clipboard.writeText(t); alert('Token list copied! You can paste in Excel/Notes to save')" style="background:#7c3aed;padding:10px;font-size:11px;margin-top:10px">COPY LIST / SAVE</button></div>
 <button onclick="if(confirm('⚠️ RESET everything? All votes + tokens will be deleted!')){fetch('/admin/reset?key=${req.query.key}',{method:'POST'}).then(()=>{alert('Reset done!'); location.href='/admin?key=${req.query.key}'})}" style="background:#fef2f2;color:#dc2626;border:2px solid #fecaca;margin-top:8px">🗑️ RESET TOKENS & VOTES</button>
 </div></div></body></html>`);
});

app.post('/admin/reset',(req,res)=>{
 if(req.query.key!==ADMIN_KEY) return res.json({error:'Wrong key'});
